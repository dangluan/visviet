...
set :repository,  "git@github.com:dangluan/visviet.git"
set :scm, :git
...
namespace :deploy do
  ...
  task :restart, :roles => :web do
    if remote_file_exists?("#{shared_path}/pids/visviet.pid")
      text = capture("cat #{shared_path}/pids/visviet.pid")
      run "kill -s QUIT `cat #{shared_path}/pids/visviet.pid`" if text.nil? == false
    end
  ...
  task :stop, :except => { :no_release => true } do
    run "kill -s QUIT `cat #{shared_path}/pids/visviet.pid`"
  end  
